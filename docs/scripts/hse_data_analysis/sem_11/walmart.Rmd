...
$$next_Weekly_Sales = w_0 + w_1 Weekly_Sales$$.
...
walmart <- walmart %>%
  arrange(Date) %>%
  group_by(Store, Dept) %>%
  mutate(leadIsHoliday = lead(IsHoliday)) %>%
  ungroup()
...
train_X <- train %>%
  ...
  select(-next_Weekly_Sales, Date) %>%
  mutate(Weekly_Sales = log(Weekly_Sales))
train_y <- log(train$next_Weekly_Sales)

test_X <- test %>%
  mutate(Month = month(Date)) %>%
  select(-next_Weekly_Sales, Date) %>%
  mutate(Weekly_Sales = log(Weekly_Sales))
test_y <- log(test$next_Weekly_Sales)

train_X <- train_X %>%
  mutate_at(vars(Store, Date, Dept, IsHoliday, Type, Month, leadIsHoliday), .funs = as.factor)
...
model <- catboost.train(train_pool,
                        ...
                        params = list(loss_function = 'RMSE',
                                      iterations = 600, metric_period=50,
                                      learning_rate=0.01,
                                      depth=8))

train$predict_catboost <- exp(catboost.predict(model, train_pool))
test$predict_catboost <- exp(catboost.predict(model, test_pool))

mae(train$next_Weekly_Sales, train$predict_catboost)
mae(test$next_Weekly_Sales, test$predict_catboost)

mape(train$next_Weekly_Sales, train$predict_catboost)
mape(test$next_Weekly_Sales, test$predict_catboost)

ggplot(test, aes(x = predict_catboost, y = next_Weekly_Sales)) +
  geom_point() +
