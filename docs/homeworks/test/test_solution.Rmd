---
title: "Контрольная работа: решение"
output: 
  html_document:
    highlight: pygments
editor_options: 
...
```{r}
df %>% 
  select(confirmed_cases, fatalities) %>% 
  summary()
```
...
```{r}
df %>%
  filter(country == "Russia", date == '2020-03-30')
```
...
df %>%
  ...
  filter(country == "US") %>%
  gather(key = "type", value = "count", confirmed_cases, fatalities) %>%
  ggplot(aes(x = date, y = count, color = type)) + 
  geom_line() + 
  labs(title = "Количество подтвержденных/фатальных случаев в США",
       x = "Дата",
...
df %>%
  ...
  summarise(confirmed_cases = sum(confirmed_cases),
            fatalities = sum(fatalities)) %>%
  filter(country %in% c("US", "China", "Russia", "Spain")) %>%
  ggplot(aes(x = date, y = confirmed_cases, color = country)) +
  geom_line() + 
  labs(x = "Дата",
       y = "Количество подтвержденных случаев",
       color = "Страна")
```
